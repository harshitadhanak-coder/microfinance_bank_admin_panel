...
import { Briefcase, Loader, Plus, Upload, Trash2, Lock, Pencil } from '../../components/icons';
...
export default function EmployeeDetailModal({ employeeId, canManage, onClose, initialTab }: Props) {
  ...
  const refreshAccount = () => qc.invalidateQueries({ queryKey: ['/employees', employeeId, 'account'] });

  /** Runs a suffixed /account POST (create / reset-default / activate / … ). */
  const accountAction = useMutation({
    mutationFn: (action: string) => api.post(`/employees/${employeeId}/account${action ? `/${action}` : ''}`),
  ...
  const updateEmail = useMutation({
    mutationFn: (email: string) => api.patch(`/employees/${employeeId}/account/email`, { email }),
              ...
              <>
                {accountQuery.isLoading ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
                    <p className="muted">No login account yet. Create a login — the employee signs in with the default password and changes it on first login.</p>
                    {canAccount && (
                      <button type="button" disabled={accountAction.isPending} onClick={() => accountAction.mutate('')}>
                        {accountAction.isPending ? 'Working…' : 'Create login account'}
                      </button>
                    )}
                  </div>
                ) : (
                        ...
                        <dd>
                          {editingEmail ? (
                            <span style={{ display: 'inline-flex', gap: '0.4rem', alignItems: 'center', flexWrap: 'wrap' }}>
                              <input
                                type="email" value={emailDraft} autoFocus style={{ minWidth: '15rem' }}
